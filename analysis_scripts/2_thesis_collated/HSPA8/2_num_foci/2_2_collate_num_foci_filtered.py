...
#non-normalised check
checks=[]
for experiment, df in updated_filtered_all.groupby('Experiment'):
       for treatment, df1 in df.groupby('treatment'):
        num_fibs=len(df1)
        num_pixels=sum(df1['lengths (pixel)'])
        med_foci_100=np.median(df1['foci_per_pixel'])*100
        check=[experiment, treatment, num_fibs, num_pixels, med_foci_100]
        checks.append(check)
checks=pd.DataFrame(checks, columns=['exp','treat','num_fibs', 'num_pixels', 'med_foci_100'])
checks.sort_values('treat')

new_normed.to_csv(f'{output}filtered_normalised_by_exp.csv')


#normalising with log, and multiplying by 100 as they're usually less than 0
updated_filtered_all['log10_foci*100']=np.log10(updated_filtered_all['foci_per_pixel']*100)
updated_filtered_all.to_csv(f'{data_output}foci_per_pix_norm_log.csv')
updated_filtered_all['maxi_exp']=max(updated_filtered_all['exp_foci'])
updated_filtered_all['norm_exp_foci']=updated_filtered_all['exp_foci']/updated_filtered_all['maxi_exp']
...
fig, ax = plt.subplots(figsize=(10,10))
ax=sns.boxplot(x='treatment', y='log10_foci*100', data=updated_filtered_all, palette='PuOr')
plt.title(f'foci per length unit fibril (pixel)(log10 of foci)')
plt.ylabel('Treatment')
plt.ylabel('normalised HSPA8 density (log10 foci per pixel)')
plt.ylim(0,2)
plt.xlabel(f'Treatment')
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig(f'{output}/log_foci_per_length_unit_colocalised_boxplot.png')

plt.savefig(f'{output}/log_foci_per_length_unit_colocalised_boxplot.svg')


conditions_list=['A8-ATP-','JB1-A8-','JB1-A8-110-5nM-', 'JB1-A8-110-0.5uM-','JB1-A8-110-2uM-','JB1-A8-SOD-0.5uM-']
select=updated_filtered_all[updated_filtered_all['treatment'].isin(conditions_list)]
select.to_csv(f'{output}lognorm_filtered_foci_per_pixel.csv')


fig, ax = plt.subplots(figsize=(5,6))
plt.rcParams.update({'font.size': 12})
ax = sns.boxplot(x='treatment', y='log10_foci*100', data=select,
...
plt.tight_layout()
plt.savefig(f'{output}/norm_log_filtered_foci_per_length_unit.png')
plt.savefig(f'{output}/norm_log_filtered_foci_per_length_unit.svg')
plt.show()


#and when it's not logged

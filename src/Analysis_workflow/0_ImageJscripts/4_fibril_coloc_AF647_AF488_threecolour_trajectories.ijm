macro "This is the same as 'fibril_coloc_AF647_AF488_threecolour macro, but also gets trajectories" {
...
Hsp_image = fib_488_output + "Background_corrected_Hsp.tif";
...
selectWindow("AF647");
run("32-bit");
run("Beam Profile Correction", "electronic_offset=750 background_image=Background_AF647");
setMinAndMax(0, 65536);
run("16-bit");
roiManager("Select", 2);
...
//no alignment of Hsp channel, just background correction etc
...
trajectories_output = Fibcolocalpath + "/Trajectories/"
print(trajectories_output);
File.makeDirectory(trajectories_output);

fibs_AF488_trajectories_output = trajectories_output + "AF488/"
print(fibs_AF488_trajectories_output);
File.makeDirectory(fibs_AF488_trajectories_output);


fibs_AF647_trajectories_output = trajectories_output + "AF647/"
print(fibs_AF647_trajectories_output);
File.makeDirectory(fibs_AF647_trajectories_output);


//now find the coordinates of the af488 LABELLED protein colocalised with fibrils, and get the trajectories at these coordinates
open(Hsp_image)
open(fib_488_output + "Fibril_colocalisation_only.csv");
...
selectWindow("Results");
        saveAs("Results", fibs_AF488_trajectories_output + Imagetitle + "_af488_colocal_traj.csv");
        run("Close");;
roiManager("reset");
...
//now find the coordinates of the AF647 labelled proteins colocalised with fibrils, and get the trajectories at these coordinates

open(bg_AF647)
open(output_647 + "Fibril_colocalisation_only.csv");
Table.rename("Fibril_colocalisation_only.csv", "Results");
runMacro("C:/Users/ljr979/Desktop/Fiji.app/plugins/Macros/v2_results_to_ROI_X2.txt");
selectWindow("Results");
        run("Close");
runMacro("C:/Users/ljr979/Desktop/Fiji.app/plugins/Macros/peak_intensity.txt"); // you need to put the location of your script
selectWindow("Results");
        saveAs("Results", fibs_AF647_trajectories_output + Imagetitle + "_af647_colocal_traj.csv");
        run("Close");;
roiManager("reset");
...
selectWindow("MAX_Hsp-1.tif");
        run("Close");
selectWindow("fibril");
        run("Close");
        selectWindow("Background_Hsp");
        run("Close");

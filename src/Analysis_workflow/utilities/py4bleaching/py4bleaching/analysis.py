...
def calculating_stoichiometries(clean_data, step_sizes):
    ...
    if len(molecule_counts['molecule_number'].unique()[0].split('_'))==9:
        print('split = 9')
        molecule_counts[['datatype', 'Experiment_number', 'protein1', 'variable1', 'variable2',  'coordsX','coordsY', 'Contour_ID', 'molecule_number']] = molecule_counts['molecule_number'].str.split('_', expand = True)

    if len(molecule_counts['molecule_number'].unique()[0].split('_'))==4:
        print('split = 4')
        molecule_counts[['variable1', 'variable2', 'Protein', 'molecule_number']] = molecule_counts['molecule_number'].str.split('_', expand = True)
    
    if len(molecule_counts['molecule_number'].unique()[0].split('_'))==6:
        print('split = 6')
        molecule_counts[['colocalisation','Experiment_number','variable1', 'variable2', 'Protein', 'molecule_number']] = molecule_counts['molecule_number'].str.split('_', expand = True)
...
def plotting_molecule_size(step_sizes, molecule_counts, output_folder):
    ...
    for size_type, size in step_sizes.items():
        ...
        for group, df in molecule_counts.groupby(['variable1', 'variable2']): 
            fig, ax = plt.subplots()
...
def sanity_checks(clean_data):
    ...
    if len(test_Df['molecule_number'].unique()[0].split('_'))==4:
        test_Df[['variable1', 'variable2', 'protein', 'molecule_number']] = test_Df['molecule_number'].str.split('_', expand = True)

    if len(test_Df['molecule_number'].unique()[0].split('_'))==9:
        test_Df[['datatype', 'Experiment_number', 'protein1', 'variable1', 'variable2', 'coordsX','coordsY', 'Contour_ID', 'molecule_number']] = test_Df['molecule_number'].str.split('_', expand = True)

    if len(test_Df['molecule_number'].unique()[0].split('_'))==6:
        test_Df[['colocalisation', 'Experiment_number', 'protein1', 'variable1', 'variable2', 'molecule_number']] = test_Df['molecule_number'].str.split('_', expand = True)
    
    test_Df['timepoint']=test_Df['timepoint'].astype(int)
    sns.lineplot(data=test_Df, x='timepoint', y='intensity', hue='variable2')
    plt.show()

    for treatment, df in test_Df.groupby('variable2'):     
...
def pipeline(input_folder,output_folder, probability_threshold, model_name, x_norm):
    ...
    molecule_counts.groupby(['variable1', 'variable2', 'Protein']).median().reset_index().to_csv(f'{output_folder}calculate_molecule_size/molecule_counts_median.csv')
